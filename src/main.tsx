...
import { FileUp, Pause, Play, RotateCcw, Volume2, VolumeX } from 'lucide-react';
...
function App() {
  ...
  const [notes, setNotes] = useState<Note[]>(demoNotes), [playing, setPlaying] = useState(false), [bpm, setBpm] = useState(92), [volume, setVolume] = useState(72), [muted, setMuted] = useState(false), [elapsed, setElapsed] = useState(0), [loadedName, setLoadedName] = useState('Demo arrangement');
  const startRef = useRef(0), pausedRef = useRef(0), notesRef = useRef(notes); notesRef.current = notes;
  ...
  return <main className="app"><header className="topbar"><div className="brand"><span className="brand-mark">MV</span><div><strong>MuVisual</strong><span>PIANO ROLL STUDIO</span></div></div><div className="session"><span className="status-dot" />SESSION 04 <span className="divider" />{loadedName}</div><label className="upload"><FileUp size={16} /> IMPORT MIDI<input type="file" accept=".mid,.midi" onChange={e => e.target.files?.[0] && upload(e.target.files[0])} /></label></header><section className="workspace"><div className="canvas-wrap"><canvas ref={canvasRef} /><div className="canvas-label"><span>LIVE VISUALIZER</span><span>{Math.floor(elapsed / 60).toString().padStart(2, '0')}:{Math.floor(elapsed % 60).toString().padStart(2, '0')} / {Math.floor(duration / 60).toString().padStart(2, '0')}:{Math.floor(duration % 60).toString().padStart(2, '0')}</span></div></div><aside className="controls"><div className="eyebrow">PLAYBACK CONTROL</div><div className="transport"><button className="primary" onClick={toggle} aria-label={playing ? 'Pause' : 'Play'}>{playing ? <Pause size={20} /> : <Play size={20} fill="currentColor" />}</button><button onClick={reset} aria-label="Reset"><RotateCcw size={18} /></button><div className="transport-copy"><strong>{playing ? 'Playing arrangement' : 'Ready to play'}</strong><span>{bpm} BPM · {notes.length} notes</span></div></div><div className="control"><div><span>TEMPO</span><b>{bpm}<small>BPM</small></b></div><input type="range" min="30" max="300" value={bpm} onChange={e => setBpm(+e.target.value)} /></div><div className="control"><div><span>MASTER VOLUME</span><b>{muted ? 0 : volume}<small>%</small></b></div><input type="range" min="0" max="100" value={volume} onChange={e => setVolume(+e.target.value)} /></div><button className="mute" onClick={() => setMuted(!muted)}>{muted ? <VolumeX size={17} /> : <Volume2 size={17} />}{muted ? 'UNMUTE AUDIO' : 'MUTE AUDIO'}</button><div className="legend"><div><i className="left" />LEFT HAND</div><div><i className="right" />RIGHT HAND</div></div></aside></section><footer><span>SMPLR AUDIO ENGINE · @TONEJS/MIDI PARSER</span><span>88 KEYS · A0 — C8</span></footer></main>;
}
